...
public class TalentsController : ControllerBase
{
    ...
    public TalentsController(GameDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<ActionResult<IEnumerable<TalentDto>>> GetTalents()
    {
        try
        {
            Console.WriteLine("Starting fetching talents...");
            var talents = await _context.Talents
                ...
                .Select(t => new TalentDto
                {
                    ...
                    TalentTypes = t.TalentTypes
                        .Select(tt => new TalentTypeDto { Id = tt.Id, Name = tt.Name })
                        .ToList(),
                    Requirements = t.Requirements
                        .Select(r => new RequirementDto { Id = r.Id, Attribute = r.Attribute, Value = r.Value })
                        .ToList()
                })
                .ToListAsync();
            Console.WriteLine($"Fetched {talents.Count} talents");
            foreach (TalentDto t in talents)
            {
                ...
                var types = string.Join(", ", t.TalentTypes.Select(tt => tt.Name));
                Console.WriteLine($"TalentTypes: {types}");
                var reqs = string.Join(", ", t.Requirements.Select(r => $"{r.Attribute}={r.Value}"));
                Console.WriteLine($"Requirements: {reqs}");
            }
        ...
        {
            return StatusCode(500, new { error = $"Error retrieving talents: {ex.Message}" });
        }
    }
